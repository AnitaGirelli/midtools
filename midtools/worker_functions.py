...
def _xarray2numpy(dtype="float32"):
    def convert(func):
        @wraps(func)
...
def _calibrate_worker(calibrator, corrections):
    def calibrate(func):
        ...
        def wrapper(data, *args, **kwargs):
            ...
            worker_corrections = list(
                dict(
                    filter(
                        lambda x: x[1] and x[0] in corrections,
                        calibrator.worker_corrections.items(),
                    )
                ).keys()
            )
...
def _to_asics(data, reverse=False, subshape=(64, 64)):
    ...
    if data.ndim == 3:
        if not reverse:
            return (
                data.reshape(-1, nv, nrows, nh, ncols)
                .swapaxes(2, 3)
                .reshape(-1, 16, nrows, ncols)
            )
        else:
            return (
                data.reshape(-1, nv, nh, nrows, ncols)
                .swapaxes(2, 3)
                .reshape(-1, 512, 128)
            )
    elif data.ndim == 2:
        if not reverse:
            return (
                data.reshape(nv, nrows, nh, ncols)
                .swapaxes(1, 2)
...
def _asic_commonmode_worker(frames, mask, adu_per_photon=8.9, subshape=(64, 64)):
    ...
    frames[:, ~mask] = np.nan
    asics = _to_asics(frames.reshape(-1, 512, 128), subshape=subshape).reshape(
        -1, *subshape
    )
...
def _cell_commonmode_worker(frames, mask, adu_per_photon=8.9, window=16):
    ...
    frames = np.where(move_mean <= (adu_per_photon / 2), frames - move_mean, frames)
    return frames
