...
class Calibrator:
    ...
    def flatfield_run_number(self, number):
        if number is None:
            ...
            raise ValueError(
                "Flatfield input parameter could not be processed:\n" f"{number}"
            )
    ...
    def stripes(self, stripes):
        if isinstance(stripes, str):
            ...
            if stripes.shape != (16, 512, 128):
                raise ValueError(
                    "Stripe mask has wrong shape: " f"shape is {stripe.shape}"
                )
    ...
    def _slice(self, arr):
        ...
        if not self.is_proc:
            ...
            arr = arr.rename({"dim_1": "dim_0", "dim_2": "dim_1"})
    ...
    def _baseline(self, arr):
        ...
        module_median = module_median.where(
            (np.isfinite(module_median) & (module_median < 0.5 * self.adu_per_photon)),
            0,
        )
    ...
    def _asic_commonmode_xarray(self, arr):
        ...
        arr = arr.assign_coords(
            asc_0=("dim_0", np.repeat(np.arange(1, nv + 1), nrows)),
        ...
        asic_median = asic_median.where(asic_median < self.adu_per_photon / 2, other=0)
        asics -= asic_median
...
def _create_mask_from_dark(dark_counts, dark_variance, pvals=(0.2, 0.5)):
    ...
    for idark in range(ndarks):
        for ii, (image, p) in enumerate(
            zip([dark_counts[idark], dark_variance[idark] / dark_counts[idark]], pvals)
        ):
...
def _create_mask_from_flatfield(
    counts, variance, average_limits=(4000, 5800), variance_limits=(200, 1500)
):
    ...
    ffmask[(counts < average_limits[0]) | (counts > average_limits[1])] = 0
    ffmask[(variance < variance_limits[0]) | (variance > variance_limits[1])] = 0
